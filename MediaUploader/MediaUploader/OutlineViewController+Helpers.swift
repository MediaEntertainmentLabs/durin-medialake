//
//  OutlineViewController+Helpers.swift
//  MediaUploader
//
...
extension OutlineViewController {
    ...
    class func fileSystemNode(from name: String, isFolder : Bool) -> Node {
        let node = Node()
        node.url = nil
    ...
    // Returns a generic node (folder or leaf) from a given url.
    class func leafNode(from item: String) -> Node {
        let node = Node()
        node.url = nil
        ...
        // Figure out the node's name based on the URL.
        node.title = item //url.localizedName
